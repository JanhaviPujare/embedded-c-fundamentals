    ...
    for( i = 0 ; i < max_record ; i++)
    {
        if( ! record[i].rollNumber)
        {
                ...
                if(! is_exist)
                {
                    ...
                    printf("Enter the studentDOB (DD/MM/YYYY) : ");
                    scanf(" %14[^\n]", record[i].DOB);    // space before %, no &, no trailing s
                    
                    printf("Enter the studentBranch     : ");
                    scanf(" %50[^\n]", record[i].branch); // space before %, no trailing s
                    
                    printf("Enter the studentName       : ");
                    scanf(" %50[^\n]", record[i].name);   // space before %, no trailing s
